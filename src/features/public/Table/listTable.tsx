...
function Filter({ column }: { column: Column<any, unknown> }) {
  ...
  return filterVariant === 'range' ? (
    ...
    <select
      className="select h-8 inline-block w-auto"
...
function DebouncedInput({
  ...
  return (
    <label className="input h-8 w-30">
    ...
    <section className="mt-10 space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="dropdown dropdown-bottom dropdown-end">
          ...
          <ul
            tabIndex={0}
            className="dropdown-content menu bg-base-100 rounded-box z-1 w-52 p-2 shadow-sm max-h-55 overflow-y-auto"
          >
            <li>
      ...
      <div>
        <Menu id={MENU_ID}>
          {contextMenu.map((c, index) => (
            <Item key={index} onClick={c.handle}>
              {c.label}
            </Item>
          ))}
        </Menu>
